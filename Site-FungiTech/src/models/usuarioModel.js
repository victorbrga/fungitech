...
function autenticar(cpf, senha) {
    ...
    var instrucaoSql = `
        SELECT cpf, email, nomeUsuario, senha, fkCnpj FROM funcionario WHERE cpf = '${cpf}' AND senha = '${senha}';
    `;
...
function cadastrar_funcionario(cpf, email, nome, senha, CNPJEmpresa) {
    ...
    var instrucaoSql = `
        INSERT INTO funcionario (cpf, email, nomeUsuario, senha, fkCnpj) VALUES ('${cpf}', '${email}', '${nome}', '${senha}', '${CNPJEmpresa}');
    `;
    console.log("Executando a instrução SQL: \n" + instrucaoSql);
    return database.executar(instrucaoSql);
}
